...
export default function InternReminderBell({ internId }) {
  ...
  const fetchReminders = async () => {
    try { setReminders(await getReminders({ intern: internId })); } catch { /* ignore */ }
  };
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)} className={`relative p-2 bg-white rounded-full shadow hover:shadow-md transition-all duration-200 ${shake ? 'animate-shake' : ''}`}>
